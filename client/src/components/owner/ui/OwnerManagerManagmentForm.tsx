...
const createManagerSchema = (hasId: boolean) => {
...
// Update the type
type ManagerFormData = z.infer<ReturnType<typeof createManagerSchema>>;
...
export default function OwnerManagerManagmentForm({
  setManagers,
  managers,
  setShowAddForm,
}: {
  ...
  const {
    ...
    resolver: zodResolver(createManagerSchema(!!id)),
    defaultValues: {
      name: "",
      email: "",
      phone: "",
  ...
  const onSubmit = async (data: ManagerFormData) => {
    try {
      ...
      toast.loading(id ? "Updating manager..." : "Adding manager...", {
        id: "addingManager",
      });
      ...
      const path = id ? `/api/managers/update/${id}` : "/api/managers/register";
      ...
      if (response.data.success) {
        ...
        if (id) {
          setManagers(
            managers.map((m) => (m.id == id ? response.data.manager : m)),
          );
        } else {
      ...
      toast.error(
        `Failed to ${id ? "update" : "add"} manager. Please try again.`,
      );
    } finally {
      toast.dismiss("addingManager");
    }
  };
  ...
  useEffect(() => {
    if (id) {
      const manager = managers.find((b) => b.id == id);
      if (manager) {
        console.log(manager);
        reset({
          name: manager.name,
          email: manager.email,
          building: manager.manager.buildingId,
          phone: manager.manager.phone,
          salary: manager.manager.salary,
          status: manager.status,
        });
        setOldImage(manager.manager.avatar);
        setOldCnic(manager.manager.cnic);
        setExtraFields(manager.manager.extraFields || []);
      }
    }
  }, [searchParams, reset]);
        ...
        >
          ...
          <div className="md:col-span-2 flex gap-3">
            ...
            <button
              type="button"
              onClick={() => {
                (setShowAddForm(false), removeParam("id"));
              }}
